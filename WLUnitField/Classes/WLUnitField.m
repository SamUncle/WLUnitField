...
- (void)drawRect:(CGRect)rect {
    ...
    if (_showBottomLine) {
        [self _drawLine:rect unitSize:unitSize];
        [self _drawTrackLine:rect unitSize:unitSize];
    }
...
/**
 ...
 @param unitSize 单个 input unit 占据的尺寸
 */
- (void)_drawLine:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    for (int i = 0; i < _inputUnitCount; ++i) {
        ...
        CGContextAddLineToPoint(_ctx, ((i + 1) * (unitSize.width) + i * _unitSpace), unitSize.height);
    }
...
/**
 ...
 @param unitSize 单个 input unit 占据的尺寸
 */
- (void)_drawTrackLine:(CGRect)rect unitSize:(CGSize)unitSize {
